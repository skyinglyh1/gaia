...
func InitTestnet(cmd *cobra.Command, config *tmconfig.Config, cdc *codec.Codec,
	...
	for i := 0; i < numValidators; i++ {
		...
		acctInfo, err := kb.Get(nodeDirName)
...
func initGenFiles(cdc *codec.Codec, mbm module.BasicManager, chainID string,
	accts exported.GenesisAccounts, genFiles []string, numValidators int) error {

	appGenState := mbm.DefaultGenesis()

	//// set the accounts in the genesis auth state
	//appGenState = genutil.SetGenesisStateInAppState(cdc, appGenState, genesisState)
	authGenState := auth.NewGenesisState(auth.DefaultParams(), accts)
	appAuthStateJson, err := codec.MarshalJSONIndent(cdc, authGenState)
	appGenState[auth.ModuleName] = appAuthStateJson
	//appGenState = genutil.SetGenesisStateInAppState(cdc, appGenState, authGenState)
