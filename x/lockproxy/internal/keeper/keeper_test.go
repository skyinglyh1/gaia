...
func setupTestInput() testInput {
	...
	lockKeyStore := sdk.NewKVStoreKey(types.ModuleName)
	...
	syncKeyStore := sdk.NewKVStoreKey(headersync.ModuleName)
	...
	ms.MountStoreWithDB(syncKeyStore, sdk.StoreTypeIAVL, newDb)
	ms.MountStoreWithDB(lockKeyStore, sdk.StoreTypeIAVL, newDb)
	...
	return testInput{cdc: cdc, ctx: ctx, authKeeper: authKeeper, syncKeeper:syncKeeper, lockKeeper:lockKeeper, supplyKeeper:supplyKeeper, bankKeeper:bankKeeper}
}
...
func Test_Keeper_CreateCoins(t *testing.T) {
	...
	fmt.Printf("lockKeeper.GetModuleAccount = %v", input.lockKeeper.GetModuleAccount(ctx).String())


	addr := sdk.AccAddress([]byte(types.ModuleName))
	acc := input.authKeeper.NewAccountWithAddress(ctx, addr)
	moduleAcc := input.lockKeeper.GetModuleAccount(ctx)
	if moduleAcc == nil {
	...
	err = input.lockKeeper.CreateCoins(ctx, creator, coins)
	if err != nil {
		t.Errorf("CreateCoins error:%v", err)
	}

	operator := input.lockKeeper.GetOperator(ctx)
	...
	storedSupply := input.supplyKeeper.GetSupply(ctx)
	total := storedSupply.GetTotal()

	assert.Equal(t, total.IsEqual(coins), true)
...
func Test_Keeper_BindProxyAndAssetHash(t *testing.T) {
	...
	err = input.lockKeeper.CreateCoins(ctx, creatorAcct.GetAddress(), coins)
	if err != nil {
		t.Errorf("CreateCoins error:%v", err)
	}
	...
	input.lockKeeper.BindProxyHash(ctx, 3, proxyHash)
	storedProxyHash := input.lockKeeper.GetProxyHash(ctx, 3)
	...
	err = input.lockKeeper.BindAssetHash(ctx, "ont", 3,  assetHash, crossedLimit, true)
	assert.Nil(t, err)

	storedAssetHash := input.lockKeeper.GetAssetHash(ctx, "ont", 3)
	assert.Equal(t, assetHash, storedAssetHash)

	storedCrossedAmount := input.lockKeeper.GetCrossedAmount(ctx, "ont", 3)
	assert.Equal(t, storedCrossedAmount.String(), sdk.NewInt(1000000000).String())

	storedCrossedLimit := input.lockKeeper.GetCrossedLimit(ctx, "ont", 3)
	...
	err = input.lockKeeper.BindAssetHash(ctx, sourceAssetDenom, 3,  assetHash1, crossedLimit1, false)
	assert.Nil(t, err)

	storedAssetHash1 := input.lockKeeper.GetAssetHash(ctx, sourceAssetDenom, 3)
	assert.Equal(t, assetHash1, storedAssetHash1)

	storedCrossedAmount1 := input.lockKeeper.GetCrossedAmount(ctx, sourceAssetDenom, 3)
	assert.Equal(t, storedCrossedAmount1.String(), sdk.NewInt(0).String())

	storedCrossedLimit1 := input.lockKeeper.GetCrossedLimit(ctx, sourceAssetDenom, 3)
...
func Test_Keeper_Lock(t *testing.T) {
	...
	assetHash1 := []byte{4, 3, 2, 1}
	err = input.lockKeeper.BindAssetHash(ctx, "ont", 3,  assetHash1, sdk.NewInt(1000000000), false)
	assert.Nil(t, err)


	toAddressBs := []byte{1, 1, 1, 1}
	amount := sdk.NewInt(10)
	err = input.lockKeeper.Lock(ctx, creatorAcct.Address, "ont", 3, toAddressBs, amount)
	assert.Nil(t, err)
}
...
func Test_Keeper_ProcessCrossChainTx(t *testing.T) {
	...
	err = input.syncKeeper.SyncGenesisHeader(ctx, mcSerializedHeader0Bs)
	if err != nil {
	...
	height := uint32(3307)
	proof3306 := ""
	err = input.lockKeeper.ProcessCrossChainTx(ctx, fromChainId, height, proof3306, header3307Bs)
	assert.Nil(t, err)
