...
func ErrSendCoinsFromModuleFail(amt sdk.Coins, fromAddr sdk.AccAddress, toAcct sdk.AccAddress) error {
	return sdkerrors.Wrap(ErrSendCoinsToModuleFailType, fmt.Sprintf("send coins:%s from Module account:%s to receiver account:%s error", amt.String(), fromAddr.String(), toAcct.String()))
}
