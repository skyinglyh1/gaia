...
const (
	...
	ip = "tcp://172.168.3.94:26657"
	operatorWalletPath = "./operator"
	operatorPwd = "11111111"
...
func Test_SendTxHeaderSync(t *testing.T) {
	...
	fromPriv, fromAddr, err := GetCosmosPrivateKey("./dev_operator", []byte(operatorPwd))
	if err != nil {
		t.Errorf("err = %v ", err)
	}
	fmt.Printf("acct = %v\n", fromAddr.String())
	...
	msg := new(headersync.MsgSyncHeadersParam)
	msg.Syncer = fromAddr
	header1, _ := hex.DecodeString(header0)
	msg.Headers = [][]byte{header1}
...
func Test_CreateCoins(t *testing.T) {
	...
	creator := fromAddr
	coins := sdk.NewCoins([]sdk.Coin{sdk.NewCoin("hahacoin", sdk.NewInt(10000)), sdk.NewCoin("hehecoin", sdk.NewInt(10000))}...)
	msg := lockproxy.NewMsgCreateCoins(creator, coins)
	if err := sendMsg(client, fromAddr, fromPriv, appCdc, msg); err != nil {
		t.Errorf("sendMsg error:%v", err)
	}
...
func Test_AssetDenomToHash(t *testing.T) {
	toAssetHash := lockproxy.DenomToHash("stake")
	fmt.Printf("stake asstHash = %s\n", hex.EncodeToString(toAssetHash))
}
...
//	//msg.GenesisHeader, _ = hex.DecodeString(genesisHeaderStr)
//
//	msg := lockproxy.NewMsgProcessCrossChainTx()
//
//	if err := sendMsg(client, fromAddr, fromPriv, appCdc, msg); err != nil {
//		t.Errorf("sendMsg error:%v", err)
//	}
